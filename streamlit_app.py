...
if run:
    ...
    if output.get("source") != "agent":
        st.caption(
            "This public demo runs keyless in Offline mode: the matching, gap "
            "detection, and broadcasts are identical to the live agent — only the "
            "message wording is templated instead of LLM-drafted. The full Strands "
            "Agents SDK + Amazon Bedrock path (which writes the messages with "
            "Claude) runs locally with `python run_agent.py` and is shown in the "
            "demo video."
        )
