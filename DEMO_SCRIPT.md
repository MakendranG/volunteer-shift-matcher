...
| 6 | 3:50–4:20 | Slide: recap 3 bullets — "Auditable matching (Python tool) • Warm outreach (LLM) • Flags the gaps, doesn't hide them." Then: "Built with the Strands Agents SDK. MIT licensed. Repo: <your public URL>." | *(Close)* "So: deterministic, auditable matching from a Strands tool; warm, human outreach from the model; and it's honest about the shifts it can't fill. It takes a job that eats a coordinator's evening and does it in seconds. Built with the Strands Agents SDK. Thanks for watching." |
